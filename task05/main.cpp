...
void wdw_spring_3d(
  ...
  Eigen::Vector3f unit_direction = (node2xyz[0] - node2xyz[1]).normalized();

  dw[0] = stiffness * C * unit_direction;
  dw[1] = - stiffness * C * unit_direction;
}
...
int main() {
  constexpr float learning_rate = 1.5e-3f;
