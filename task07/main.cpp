...
void solve_laplace_gauss_sidel_on_grid(
  ...
  for (unsigned int ix = 0; ix < grid_size; ++ix) {
    for (unsigned int iy = 0; iy < grid_size; ++iy) {
      ...
      float left = vtx2val[idx_center - 1];
      float right = vtx2val[idx_center + 1];
      float up = vtx2val[idx_center - grid_size];
      float down = vtx2val[idx_center + grid_size];
      vtx2val[idx_center] = 0.25f * (left + right + up + down);
    }
  }
}
